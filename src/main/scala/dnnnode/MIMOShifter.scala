...
class MIMOShifterIO[T <: Data](private val gen: T, val entries: Int, NumIns: Int) extends Bundle
{
...
//  val deq = Flipped(DeqIO(Vec(NumOuts, gen)))
  ...
  val idx = Input(UInt(log2Ceil(entries + 1).W))
  val numDeq = Input(UInt(log2Ceil(entries + 1).W))
...
class MIMOShifter[T <: Data](gen: T,
...
//  require(entries % NumOuts == 0, "Use companion object Queue.apply for zero entries")
  ...
  val io = IO(new MIMOShifterIO(genType, entries, NumIns))
...
//  val full = ptr_match && maybe_full
  ...
  when (do_deq) {
    deq_ptr := (deq_ptr + io.numDeq) % entries.U
    when (io.clear) {
      deq_ptr := 0.U
    }
  }
...
//  io.Out.valid := !empty //(ptr_diff >= 0.U)
  when (bufCount > (io.numDeq - 1.U)) {
    io.deq.valid := true.B
  }.otherwise {
    io.deq.valid := false.B
  }

  io.deq.bits := Mux(bufCount > (io.numDeq - 1.U), ram((deq_ptr + io.idx) % entries.U), 0.U.asTypeOf(genType))
